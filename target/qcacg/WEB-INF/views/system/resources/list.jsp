	                ...
	                <table class="table table table-striped table-bordered table-hover" id="resourcesTable">
	                     ...
	                     <tbody>
	                     	<c:forEach items="${pageInfo.list}" var="tree">
		                         <tr class="treegrid-${tree.resourcesId} <c:if test="${tree.parentResourcesId != 0}">treegrid-parent-${tree.parentResourcesId}</c:if>">
		                             <td>
		                                 <input type="checkbox" class="ii-checks" name="check" value="${tree.resourcesId}">
		                             </td>
		                             <td>${tree.resourcesName}</td>
		                             <td>
		                             	<c:if test="${tree.resourcesType eq 0 }">父级菜单</c:if>
		                             	<c:if test="${tree.resourcesType eq 1 }">子级菜单</c:if>
		                             	<c:if test="${tree.resourcesType eq 2 }">菜单按钮</c:if>
		                             </td>
		                             <td>${tree.resourcesUrl}</td>
		                             <td><i class="${tree.resourcesView}"></i></td>
		                             <td>${tree.resourcesUpdate}</td>
		                         </tr>
	                         </c:forEach>
	                     </tbody>
	                 </table>
