...
(function() {
	...
	qcacg.ajaxSubmit = (function(form, params) {// form 表单ID. params ajax参数
		var pp = {
			error : function(XMLHttpRequest, textStatus, errorThrown) {
				layer.open({
					type : 1,
	...
	qcacg.ajaxOpen = (function(options) {
	 ...
	 * 增加命名空间功能
	 * 
	 * 使用方法：qcacg.ns('jQuery.bbb.ccc','jQuery.eee.fff');
	 */
	qcacg.ns = function() {
		var o = {}, d;
	...
	qcacg.msg = function (msg) {
		layer.msg(msg);
	}
	
	qcacg.toastrsAlert = function(obj){
		qcacg.init();
		if(obj.code == 'info'){
	...
	//关闭弹出窗口
	qcacg.closeWindow = function(){
		 layer.closeAll();
	}
	
	
	qcacg.confirm = function confirm(dowork){
	...
	CommnUtil = {
		...
		loadingImg : function() {
			var html = '<div class="alert alert-warning" id="loadImg">'
					...
					+ '<img src="http://static.qcacg.com/back/img/loading.gif"/><div>'
					+ '</div>';
			return html;
		},
		/**
