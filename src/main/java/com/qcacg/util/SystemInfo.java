...
public class SystemInfo
{
	public static JSONObject SystemProperty()
	{
		...
		monitorMap.put("processors", String.valueOf(r.availableProcessors()));// JVM可以使用的处理器个数
	...
	public static JSONObject memory(Sigar sigar)
	{
		...
		{
			Runtime r = Runtime.getRuntime();
			monitorMap.put("jvmTotal", div(r.totalMemory(), (1024 * 1024), 2) + "M");// java总内存
			monitorMap.put("jvmUse", div(r.totalMemory() - r.freeMemory(), (1024 * 1024), 2) + "M");// JVM使用内存
			monitorMap.put("jvmFree", div(r.freeMemory(), (1024 * 1024), 2) + "M");// JVM剩余内存
			monitorMap.put("jvmUsage", div(r.totalMemory() - r.freeMemory(), r.totalMemory(), 2));// JVM使用率

			Mem mem = sigar.getMem();
			// 内存总量
			monitorMap.put("ramTotal", div(mem.getTotal(), (1024 * 1024 * 1024), 2) + "G");// 内存总量
			monitorMap.put("ramUse", div(mem.getUsed(), (1024 * 1024 * 1024), 2) + "G");// 当前内存使用量
			monitorMap.put("ramFree", div(mem.getFree(), (1024 * 1024 * 1024), 2) + "G");// 当前内存剩余量
			monitorMap.put("ramUsage", div(mem.getUsed(), mem.getTotal(), 2));// 内存使用率
	...
	public static JSONObject usage(Sigar sigar)
	{
		JSONObject monitorMap = new JSONObject();
		try
		{
			Runtime r = Runtime.getRuntime();
			monitorMap.put("jvmUsage", Math.round(div(r.totalMemory() - r.freeMemory(), r.totalMemory(), 2) * 100));// JVM使用率

			Mem mem = sigar.getMem();
			// 内存总量
			monitorMap.put("ramUsage", Math.round(div(mem.getUsed(), mem.getTotal(), 2) * 100));// 内存使用率

			List<JSONObject> cpu = cpuInfos(sigar);
			double b = 0.0;
			for (JSONObject m : cpu)
			{
	...
	public List<JSONObject> diskInfos(Sigar sigar) throws Exception
	{
		List<JSONObject> monitorMaps = new ArrayList<JSONObject>();
