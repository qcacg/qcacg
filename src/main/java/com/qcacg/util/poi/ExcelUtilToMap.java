...
public class ExcelUtilToMap
{
	 ...
	 * @throws Exception
	 */
	public static List<Map<String, String>> readExcelFileMap(String sRealPath, int iSheetNum, String key[],
			...
			for (; j < iRowNum; j++, iCellNum = 0)
			{
				...
				for (int k = 0; k < iCellNum; k++)
				{
					...
					case Cell.CELL_TYPE_STRING: // 文本类型
						aValues.put(key[k], String.valueOf((cellTmp.getStringCellValue())));
						break;
					...
					{ // 文本类型
						...
						if (DateUtil.isCellDateFormatted(cellTmp))
						{
							...
							aValues.put(key[k], String.valueOf(df.format(dTmp)));
						} else
						{
							...
							} else
							{
								...
								aValues.put(key[k],
										String.valueOf(NumberFormat.getInstance().format(d).replace(",", "")));
							}
						}
						break;
					}
	 ...
	 * @throws Exception
	 */
	public static List<Map<String, String>> readExcelFileMap(InputStream is, int iSheetNum, String key[],
			boolean isTitle) throws Exception
	{
					...
					throws Exception
	{
		...
		if (aList != null && aList.size() > 0)
		{
			...
			for (int i = 0; i < len; i++, iCountNum++)
			{
				...
				// 获取数据
				Map<String, String> aValues = (Map<String, String>) aList.get(i);
