					...
					<li class="nav-header">
						<div class="dropdown profile-element">
							<span><img alt="image" class="img-circle"
								src="${ctx}/${USER_SESSION_KEY.userHead}" width="64px" height="64px"/></span> <a
						...
						<div class="logo-element">qcacg</div>
					</li>
					<c:forEach items="${list}" var="rs">
						<li><a href="#"> <i class="${rs.resourcesView}"></i> <span
								class="nav-label">${rs.resourcesName}</span> <span class="fa arrow"></span>
						</a>
							<ul class="nav nav-second-level">
								<c:forEach items="${rs.childrenResources}" var="ch">
									<li><a title="${ch.resourcesDescription}" class="J_menuItem"
										href="${ctx}${ch.resourcesUrl}?id=${ch.resourcesId}">${ch.resourcesName}</a></li>
								</c:forEach>
							</ul></li>
					</c:forEach>
					<!-- 该功能可选：方便管理登陆用户而已 -->
